...
class Regression():
    ...
    def preprocess_data(self):
        self.preproc_X = Pipeline([('minmax', MinMaxScaler(feature_range=(0, 1))), ('stdscaler', StandardScaler())])
        self.preproc_y = Pipeline([('minmax', MinMaxScaler(feature_range=(0, 1))), ('stdscaler', StandardScaler())])
    ...
    def build_model(self, model_type):
        ...
        if model_type != 'dl':
            ...
            # design network
            model = Sequential()
            model.add(Dense(nunits, activation='relu', input_shape=(self.train_X.shape[1],)))
    ...
    def train_model(self, model, model_type):
        ...
        if model_type != 'dl':
            ...
            model.fit(self.train_X_p, self.train_y_p, epochs=50, batch_size=16, validation_split=0.1, verbose=2, callbacks=[early_stopping_monitor], shuffle=True)
        end = time.time()
